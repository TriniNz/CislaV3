exports.run = async (Discord, client, message, args) => {
    ...
    memberToSet.roles.remove([`${roleToSet.id}`,`${adRoleSet.id}`]).then(() => {
        ...
        client.channels.cache.get(logChannelID).send(new Discord.MessageEmbed()
            .setAuthor('Staff - Remoção.')
            .setDescription('Um membro acabou de perder seu cargo.')
        ...
        message.channel.send(new Discord.MessageEmbed()
            .setDescription("❎ | Houve um erro inesperado ao remover este cargo.")
            .setFooter(`CislaV3!`)
            .setColor('#9900cc')
            .setTimestamp()
        ).then(Msg => {Msg.delete({timeout:15*1000}); message.delete({timeout:15*1000})});
        console.log("Erro no comando removestaff.\n\n " + err)
